@echo off
:: =====================================================================
::   SHRINK OVERSIZED PDF PAGES — Drag-and-Drop Tool
:: =====================================================================
::   Double-click to open a browser-based drag-and-drop tool
::   that detects oversized PDF pages and shrinks them to
::   US Letter (8.5 x 11") using Ghostscript, similar to
::   Microsoft's "Print to PDF" shrink-to-fit behavior.
::
::   - No admin rights needed
::   - First run downloads qpdf + Ghostscript (one time only)
::   - Opens in your default browser
::   - Files never leave your computer
:: =====================================================================

title Shrink Oversized Pages - Starting...

powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$lines = @(); $reading = $false; foreach ($line in (Get-Content -LiteralPath '%~f0' -Encoding UTF8)) { if ($line -eq '# >>>PS_START<<<') { $reading = $true; continue }; if ($line -eq '# >>>PS_END<<<') { break }; if ($reading -and $line.StartsWith('## ')) { $lines += $line.Substring(3) } elseif ($reading -and $line -eq '##') { $lines += '' } }; $sp = Join-Path $env:TEMP 'Shrink-Pages-Server.ps1'; $lines -join [Environment]::NewLine | Set-Content -LiteralPath $sp -Encoding UTF8; try { & $sp } finally { Remove-Item $sp -Force -ErrorAction SilentlyContinue }"
...
## # ═══════════════════════════════════════════════════════════════════
## $tempDir = Join-Path $env:TEMP "shrink-pages-work"
...
## <title>Shrink Oversized Pages</title>
## <style>
...
## </head>
## <body>
## <h1>Shrink Oversized Pages</h1>
## <p class="subtitle">Drop PDFs here to shrink oversized pages to US Letter (8.5 &times; 11&quot;)</p>
...
## </div>
## <div class="footer">
##   Powered by Ghostscript &amp; qpdf &mdash; files are processed locally and never leave your computer.<br>
##   Oversized pages are scaled to fit US Letter, similar to Microsoft Print to PDF.
## </div>
## <script>
...
##       const resp = await fetch('/shrink', { method: 'POST', body: formData });
##       const data = await resp.json();
##       if (data.success) {
##         lastOutputFolder = data.outputFolder || '';
##         const alreadyLetter = !!data.alreadyLetter;
##         results.push({ name: file.name, success: true, origSize: file.size, newSize: data.newSize, downloadUrl: data.downloadUrl, alreadyLetter });
##         updateFileItem(itemEl, { success: true, name: file.name, origSize: file.size, newSize: data.newSize, downloadUrl: data.downloadUrl, alreadyLetter });
##       } else {
...
##     <div class="file-meta">Checking page sizes\u2026 (${fmt(size)})</div></div>`;
...
##         <div class="file-meta">Already US Letter \u2014 no changes needed (${fmt(d.origSize)})</div></div>`;
##     } else {
##       const pct = d.origSize > 0 ? Math.round((1 - d.newSize / d.origSize) * 100) : 0;
##       const s = pct > 0 ? ` \u00b7 ${pct}% smaller` : pct < 0 ? ` \u00b7 ${Math.abs(pct)}% larger` : '';
##       el.innerHTML = `<div class="file-icon">\u2705</div>
##         <div class="file-info"><div class="file-name">${esc(d.name)}</div>
##         <div class="file-meta">Shrunk to Letter: ${fmt(d.origSize)} \u2192 ${fmt(d.newSize)}${s}</div></div>
...
##     <div class="summary"><strong>${ok.length}</strong> processed${fail.length ? `, <strong>${fail.length}</strong> failed` : ''}
##     \u00b7 ${fmt(tO)} \u2192 ${fmt(tN)}</div>
...
##
## Write-Host ""
## Write-Host "  Shrink Oversized Pages is running at $prefix" -ForegroundColor Cyan
...
##
##         } elseif ($path -eq "/shrink" -and $request.HttpMethod -eq "POST") {
...
##                 $outputFolder = Join-Path $tempDir "Shrunk_PDFs"
##                 if (-not (Test-Path $outputFolder)) { New-Item -ItemType Directory -Path $outputFolder -Force | Out-Null }
##                 $persistPath = Join-Path $outputFolder $filename
##
##                 # Check if any pages are oversized (not US Letter)
##                 $needsResize = $false
##                 try { $needsResize = Test-NeedsResize $inputPath } catch { }
##
##                 if (-not $needsResize) {
##                     # Already Letter-sized — copy as-is
##                     Copy-Item $inputPath $outputPath -Force
##                     Remove-Item $inputPath -Force -ErrorAction SilentlyContinue
##                     try { Copy-Item $outputPath $persistPath -Force } catch { }
##                     $newSize = (Get-Item $outputPath).Length
##                     $dlId = [System.IO.Path]::GetFileNameWithoutExtension($outputPath)
##                     $jsonResp = @{ success = $true; newSize = $newSize; alreadyLetter = $true; downloadUrl = "/download/$dlId"; outputFolder = $outputFolder } | ConvertTo-Json
##                 } elseif (Ensure-Ghostscript) {
##                     # Resize oversized pages to Letter via Ghostscript
##                     $gsResult = Resize-ToLetter -inputPath $inputPath -outputPath $outputPath
##                     Remove-Item $inputPath -Force -ErrorAction SilentlyContinue
##                     if ((Test-Path $outputPath) -and $gsResult.ExitCode -eq 0) {
##                         try { Copy-Item $outputPath $persistPath -Force } catch { }
##                         $newSize = (Get-Item $outputPath).Length
##                         $dlId = [System.IO.Path]::GetFileNameWithoutExtension($outputPath)
##                         $jsonResp = @{ success = $true; newSize = $newSize; downloadUrl = "/download/$dlId"; outputFolder = $outputFolder } | ConvertTo-Json
##                     } else {
##                         $errMsg = if ($gsResult.Error) { $gsResult.Error.Trim().Substring(0, [math]::Min(200, $gsResult.Error.Trim().Length)) } else { "Ghostscript failed with exit code $($gsResult.ExitCode)" }
##                         $jsonResp = @{ success = $false; error = $errMsg } | ConvertTo-Json
##                         Remove-Item $outputPath -Force -ErrorAction SilentlyContinue
##                     }
##                 } else {
##                     Remove-Item $inputPath -Force -ErrorAction SilentlyContinue
##                     $jsonResp = @{ success = $false; error = "Ghostscript is required for page resizing but could not be installed." } | ConvertTo-Json
##                 }
##
##             } catch {
...
##             # Serve a processed PDF for download
